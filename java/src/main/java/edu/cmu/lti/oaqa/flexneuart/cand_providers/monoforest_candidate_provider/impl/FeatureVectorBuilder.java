...
public class FeatureVectorBuilder {
    ...
    public static void main(String[] args) throws Exception {
        FeatureVectorBuilder ff = new FeatureVectorBuilder();
        ...
        ff.buildAllFeatures("/Volumes/Ex_Volume/msmarcoProcces/input_data/dev/QuestionFields.jsonl",
                ...
                "/Volumes/Ex_Volume/msmarcoProcces/itog");
    }
    ...
    private void processSplitWithSSD(Path candidatesFile, Path outputFile, Map<String, String> queryMap, RocksDB db) {
        ...
        try (BufferedReader reader = Files.newBufferedReader(candidatesFile, StandardCharsets.UTF_8);
            ...
            while ((line = reader.readLine()) != null) {
                ...
                float bm25 = (float)candJson.getDouble("bm25");
                float tfidf = (float)candJson.getDouble("tfidf");
                float rank = (float)candJson.getDouble("rank");
                ...
                if (queryText != null) {
                    float[] qf = queryVectors.get(qId);
                    float[] df = docVectors.get(docId);
                    float[] jf = factorManager.extractJointFactors(queryText, docJson.optString("title"), docJson.optString("body"), docId);

                    float[] combined = combineArrays(rank, bm25, tfidf, jf, qf, df);
    ...
    private float[] combineArrays(float rank, float bm25, float tfidf, float[] joint, float[] q, float[] d) {
        float[] combined = new float[3 + joint.length + q.length + d.length];
        combined[0] = rank; combined[1] = bm25; combined[2] = tfidf;
        int offset = 3;
