 ...
 *
 * Результаты выводятся в консоль. Отдельный TFIDFFamilyTest проверяет
 * фиксированные значения на неизменной коллекции в памяти.
 */
public class TFIDFFamilyIT extends AvailableIndexTest {
    ...
    public void verifiesFeatureValues() throws Exception {
        ...
        try (Directory directory = FSDirectory.open(Paths.get(indexPath));
            ...
            Query query = family.buildLuceneQuery(
                    TFIDFFamily.TFIDF_SCORE, queryTokens, threshold, TFIDFFamily.Comparison.GT);

            System.out.println("Индекс: " + indexPath);
            System.out.println("Поле: " + field + "; только body, без title");
            ...
            System.out.println("Условие: TF-IDF > " + threshold);
            ...
            System.out.println("doc_id\tquery\tthreshold\tlucene_tfidf\tfamily_tfidf\tstatus");
            ...
            for (ScoreDoc hit : hits.scoreDocs) {
                ...
                float current = family.calculateFeatureByName(TFIDFFamily.TFIDF_SCORE, queryTokens, document).value;
                ...
                boolean ok = aboveThreshold && sameAsLucene;
                if (ok) passed++;
            ...
            System.out.println("Нарушений TF-IDF > n: " + thresholdFailures);
            System.out.println("Расхождений со значением Lucene: " + luceneFailures);
            if (passed != hits.scoreDocs.length) {
                throw new AssertionError("TF-IDF: обнаружены расхождения, см. строки FAIL");
            }
        }
    }
